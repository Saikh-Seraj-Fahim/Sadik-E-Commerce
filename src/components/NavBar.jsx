...
export default function NavBar() {
    ...
    return <>
        <header className="sticky top-0 right-2 left-2 w-full bg-white dark:bg-gray-900/80 backdrop-blur-md border-b 
        border-gray-200 dark:border-gray-800 rounded-full">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex h-16 items-center justify-between">
                    ...
                    <nav className="hidden md:flex items-center space-x-8">
                        {navLinks.map(link => <a key={link.text} href={link.href} className="font-bold text-[#088347]   
            ...
            <div className="flex flex-col h-full">
                ...
                <nav className="flex-grow p-4">
                    <div className="flex flex-col space-y-2">
                        {navLinks.map(link => <a key={link.text} href={link.href} className="px-3 py-2 text-base font-medium text-[#088347] dark:text-gray-200 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800" onClick={() => setIsMenuOpen(false)}>
                            {link.text}
                        </a>)}
                    </div>
                </nav>
